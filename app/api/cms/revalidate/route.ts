...
export async function POST(request: NextRequest) {
  ...
  // 8. Notificar motores de busca sobre novas URLs
  let indexingResults: {
    google: { enabled: boolean; success: boolean };
    indexnow: { enabled: boolean; success: boolean };
    sitemapPing: { success: boolean };
  } | undefined;

  if (urlsToIndex.length > 0) {
    try {
      // Notificar cada URL individualmente para usar Google Indexing API
      const results = await Promise.all(
        urlsToIndex.map((path) => notifySearchEngines(path))
      );

      // Agregar resultados
      const googleSuccess = results.some((r) => r.google.success);
      const indexnowSuccess = results.some((r) => r.indexnow.success);
      const sitemapSuccess = results.some((r) => r.sitemapPing.success);

      indexingResults = {
        google: {
          enabled: results[0]?.google.enabled ?? false,
          success: googleSuccess,
        },
        indexnow: {
          enabled: results[0]?.indexnow.enabled ?? false,
          success: indexnowSuccess,
        },
        sitemapPing: { success: sitemapSuccess },
      };

      console.log(`[CMS Webhook] Motores de busca notificados:`, {
        urls: urlsToIndex.length,
        google: googleSuccess,
        indexnow: indexnowSuccess,
        sitemapPing: sitemapSuccess,
      });
    } catch (error) {
  ...
  // 9. Notificar remoção de URLs (para content.deleted)
  if (
    (event === "content.deleted" || event === "content.unpublished") &&
    content?.slug
  ) {
    try {
      const path =
        content.type === "post"
          ? `/artigo/${content.slug}`
          : `/pagina/${content.slug}`;
      const removalResult = await notifyUrlRemoval(path);
      console.log(`[CMS Webhook] URL removida notificada:`, removalResult);
    } catch (error) {
      console.error("[CMS Webhook] Erro ao notificar remoção:", error);
    }
  }
  ...
  const response = {
    ...
    indexed: urlsToIndex.length > 0
      ? {
          urls: urlsToIndex,
          searchEngines: getSearchEngineStatus(),
